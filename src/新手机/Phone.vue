...
const setupTimeListener = async () => {
  ...
  if (typeof waitGlobalInitialized === 'function') {
    try {
      ...
      console.log('[Phone] Mvu 已初始化，开始注册事件监听器');
    } catch (error) {
      console.warn('[Phone] 等待 Mvu 初始化失败:', error);
      // 即使失败也继续，因为可能已经初始化了
    }
  }
...
const loadTimeFromTavern = async () => {
  try {
    ...
    if (typeof Mvu === 'undefined') {
      console.warn('[Phone] Mvu 未定义，无法加载时间数据');
      return;
    }
