  ...
  const messages = Object.entries(activeContact.value.聊天记录)
    .map(([timeStr, msg]) => {
      ...
      if (!isNaN(time.getTime())) {
        ...
        // 解析失败，使用当前时间并记录警告
        console.warn(`[ChatPage] 无法解析时间戳: ${timeStr}`);
        timestamp = Date.now();
      }
...
function loadContactsData(contactsDataParam: Record<string, any>) {
  ...
  Object.entries(contactsDataParam).forEach(([contactName, contactInfo]) => {
    if (!contactInfo || typeof contactInfo !== 'object') {
      console.warn(`[ChatPage] 联系人 ${contactName} 数据无效`);
      return;
    }
    ...
    // 处理聊天记录
    const chatRecords = info.聊天记录 || {};
    const chatRecordCount = Object.keys(chatRecords).length;

    console.log(`[ChatPage] 处理联系人 ${contactName}:`, {
...
const loadTavernData = async () => {
  ...
  loadPromise = (async () => {
    try {
      ...
      if (typeof Mvu === 'undefined') {
        console.warn('[ChatPage] Mvu 未定义，无法加载数据');
        return;
      }
      ...
      console.log('[ChatPage] 加载到的手机数据:', {
        has_user: !!phoneData?.用户,
        user_nickname: phoneData?.用户?.昵称,
        contacts_count: phoneData?.联系人 ? Object.keys(phoneData.联系人).length : 0,
      ...
      if (phoneData?.联系人) {
        const contactNames = Object.keys(phoneData.联系人);
        console.log('[ChatPage] 开始加载联系人数据:', contactNames);
        ...
        console.warn('[ChatPage] 没有联系人数据');
      }
    } catch (error) {
      console.error('[ChatPage] 加载酒馆数据时出错:', error);
    } finally {
      loadPromise = null;
    }
  })();
...
// 数据监听和加载
const setupTavernDataListener = async () => {
  // 初始化时立即加载一次数据
  loadTavernData();

  // 等待 Mvu 初始化 - 关键！确保事件监听器一定会被注册
  if (typeof waitGlobalInitialized === 'function') {
    try {
      await waitGlobalInitialized('Mvu');
      console.log('[ChatPage] Mvu 已初始化，开始注册事件监听器');
    } catch (error) {
      console.warn('[ChatPage] 等待 Mvu 初始化失败:', error);
      // 即使失败也继续，因为可能已经初始化了
    }
  }
  ...
  if (typeof eventOn === 'function' && typeof Mvu !== 'undefined') {
    ...
    eventOn(Mvu.events.VARIABLE_UPDATE_ENDED, (variables: any) => {
      console.log('[ChatPage] 变量更新结束');
      loadTavernData();
    });
  } else {
    console.warn('[ChatPage] 无法注册 MVU 事件监听器 - Mvu 未定义');
  }

  // 监听酒馆消息事件 - 添加必要的延迟
  if (typeof eventOn === 'function') {
    console.log('[ChatPage] 注册酒馆事件监听器');
